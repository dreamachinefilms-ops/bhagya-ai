...
import BirthDetailsSettings from "@/components/settings/BirthDetailsSettings";
    ...
    <div className="relative mx-auto flex min-h-0 w-full max-w-5xl flex-col">
      ...
      <div className="mt-4 grid min-h-0 gap-5 md:grid-cols-[220px_minmax(0,1fr)]">
        ...
        <section className="min-h-0 rounded-[24px] border border-white/10 bg-[#071225]/90 p-4 shadow-2xl backdrop-blur-xl sm:p-7">
          ...
          {section === "birth" && <>{profileError && !hasLoadedProfile && <SectionError message="Your birth details could not be loaded." onRetry={() => void loadProfile()} />}{profileColdLoading ? <SettingsRowsSkeleton count={4} /> : profile?.birthDetails ? <BirthDetailsSettings profile={profile} onUpdated={updateCachedProfile} /> : <div className="space-y-5"><Heading title="Birth Details" text="Complete your initial profile before using the one-time correction." /><p className="rounded-xl border border-white/10 p-4 text-sm text-white/50">No birth profile has been saved yet. Complete the initial Birth Profile flow first.</p><Link href="/birth-details" className="inline-flex min-h-12 items-center rounded-xl bg-sky-500 px-5 font-semibold">Complete Birth Profile</Link></div>}</>}
