...
import { isValidTimeZone } from "@/lib/userPreferences";
...
export async function POST(request: Request) {
  ...
  try {
    const [profile, birth, preferences] = await Promise.all([getSavedUserProfile({ request, userId: user.id }), getSavedBirthDetails({ request, userId: user.id }), getOrCreateUserPreferences({ request, userId: user.id })]);
    ...
    const timezone = preferences.timezone || birth.timezoneId || (typeof input.timezone === "string" ? input.timezone : null) || "Asia/Kolkata";
