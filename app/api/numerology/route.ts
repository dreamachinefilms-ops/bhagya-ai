...
import {
  ...
  calculateNumerology,
  extractNumerologyDetailsFromConversation,
} from "@/lib/numerology/calculateNumerology";
import { buildNumerologyMissingResponse } from "@/lib/guidanceResponses";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const numerologyDetails =
      extractNumerologyDetailsFromConversation(conversationText);

    if (!numerologyDetails.isComplete) {
      const answer = buildNumerologyMissingResponse(
        numerologyDetails.missing,
        languageCode
      );
    ...
    const numerologyData = calculateNumerology(numerologyDetails);
